...
function getWikipediaSvgUrl(countryCode) {
  ...
  // Try a simpler URL pattern that should work
  return `https://upload.wikimedia.org/wikipedia/commons/Flag_of_${countryName}.svg`;
}

// Main function
async function downloadSvgFlagsSimple() {
  ...
  for (const code of testCodes) {
    ...
    try {
      const url = getWikipediaSvgUrl(code);
...
// Run the script
downloadSvgFlagsSimple().catch(console.error); 