(function () {
  ...
  function applyArcadeCategory(weight) {
    ...
    const textElement = scoreboardInstance.svg.getElementById('category_weight');
    if (textElement) {
      const combined = [genderLabel, weightLabel].filter(Boolean).join(' ').trim();
      scoreboardInstance.setTextForElementOrGroup(textElement, combined);
    }
  }
  ...
  function processEvent(payload) {
    ...
    switch (eventType) {
      ...
      case 'match_config': {
        ...
        applyArcadeCategory(payload.weight);
