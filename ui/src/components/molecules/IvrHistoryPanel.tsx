								...
								videos.map((v) => (
									<tr key={v.id} className={`cursor-pointer hover:bg-blue-900 ${selectedVideoIds.has(v.id)?'bg-blue-900/40':''}`} onClick={() => toggleVideoSelection(v.id)} onDoubleClick={async () => {
										...
										<td className="px-3 py-2 whitespace-nowrap">
											<Button variant="ghost" size="sm" className="text-blue-400 hover:text-blue-300 mr-2" onClick={async (e) => { e.stopPropagation(); if (v.id) { const { invoke } = await import('@tauri-apps/api/core'); await invoke('ivr_open_recorded_video', { recorded_video_id: v.id, recordedVideoId: v.id }); } }}>Open</Button>
