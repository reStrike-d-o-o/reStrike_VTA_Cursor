...
class GlobalErrorBoundary extends React.Component<{children: React.ReactNode}, {hasError: boolean, error: any}> {
  ...
  componentDidCatch(error: any, errorInfo: any) {
    ...
    // logError('Global error boundary caught:', error);
  }
  render() {
    if (this.state.hasError) {
      return (
        <div className="fixed inset-0 flex flex-col items-center justify-center bg-black bg-opacity-80 z-50">
          <div className="bg-red-800 text-white p-8 rounded-lg shadow-lg max-w-xl">
            <h2 className="text-2xl font-bold mb-4">Something went wrong</h2>
            <pre className="text-sm whitespace-pre-wrap break-all">{String(this.state.error)}</pre>
            <p className="mt-4 text-gray-300">Please check your environment configuration and reload the app.</p>
          </div>
        </div>
      );
    }
