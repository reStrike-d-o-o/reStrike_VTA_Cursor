...
use archival::{ArchiveInfo, AutoArchiveConfig, LogArchiver};
use logger::Logger;
...
impl LogManager {
    pub fn new(config: LogConfig) -> io::Result<Self> {
        ...
        let archiver =
            LogArchiver::new_with_archive_dir(config.retention_days, config.archive_dir.clone());
    ...
    pub fn create_complete_archive(&self) -> io::Result<ArchiveInfo> {
        ...
        let archive_name = archive_path
            ...
            .and_then(|n| n.to_str())
            .ok_or_else(|| {
                io::Error::new(io::ErrorKind::InvalidData, "Invalid archive filename")
            })?;
    ...
    pub async fn create_and_upload_archive(
        ...
        Ok(format!(
            "Archive '{}' uploaded successfully to Google Drive",
            archive_info.name
        ))
    }

    /// Create archive, upload to Google Drive, and delete local file
    pub async fn create_upload_and_cleanup_archive(
        &self,
    ) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        // Create the archive
        let archive_info = self
        ...
        log::info!(
            "Deleted local archive after successful upload: {}",
            archive_info.name
        );

        Ok(format!(
            "Archive '{}' uploaded to Google Drive and cleaned up locally",
            archive_info.name
        ))
    }
