...
use crate::security::{
    AccessLevel, ConfigCategory, SecureConfigManager, SecurityError, SecurityResult,
};
...
impl ConfigMigrationTool {
    ...
    pub async fn migrate_all_configurations(&mut self) -> SecurityResult<MigrationStats> {
        ...
        self.migrate_hardcoded_credentials(&session.session_id)
            .await?;
        self.migrate_frontend_stores(&session.session_id).await?;
        self.migrate_environment_variables(&session.session_id)
            .await?;
    ...
    async fn migrate_json_file(&mut self, session_id: &str, file_path: &str) -> SecurityResult<()> {
        ...
        self.extract_database_credentials(session_id, &config)
            .await?;
    ...
    async fn extract_obs_credentials(
        &mut self,
        session_id: &str,
        config: &Value,
    ) -> SecurityResult<()> {
    ...
    async fn extract_database_credentials(
        &mut self,
        session_id: &str,
        config: &Value,
    ) -> SecurityResult<()> {
        if let Some(db_config) = config.get("database") {
            if let Some(connection_string) =
                db_config.get("connection_string").and_then(|v| v.as_str())
            {
    ...
    async fn extract_network_secrets(
        &mut self,
        session_id: &str,
        config: &Value,
    ) -> SecurityResult<()> {
    ...
    async fn migrate_hardcoded_credentials(&mut self, session_id: &str) -> SecurityResult<()> {
        ...
        let hardcoded_credentials = [
            (
                ...
                "cekPIbj@245",
                ConfigCategory::ObsCredentials,
                "Default OBS password",
            ),
            (
                "obs.OBS_REC.password",
                "cekPIbj@245",
                ConfigCategory::ObsCredentials,
                "OBS_REC connection password",
            ),
            (
                "obs.OBS_STR.password",
                "cekPIbj@245",
                ConfigCategory::ObsCredentials,
                "OBS_STR connection password",
            ),
        ];

        for (key, value, category, description) in &hardcoded_credentials {
            self.config_manager
                .set_config(session_id, key, value, category.clone(), Some(description))
                .await?;
        ...
        for store_file in &store_files {
            if Path::new(store_file).exists() {
                ...
                if let Ok(content) = fs::read_to_string(store_file) {
                    self.extract_frontend_credentials(session_id, &content)
                        .await?;
                }
            }
        }
    ...
    async fn extract_frontend_credentials(
        &mut self,
        session_id: &str,
        content: &str,
    ) -> SecurityResult<()> {
    ...
    pub async fn verify_migration(&self, session_id: &str) -> SecurityResult<bool> {
        ...
        for config_key in &required_configs {
            match self
                .config_manager
                .get_config(session_id, config_key)
                .await?
            {
                Some(value) => {
                    if value.is_empty() {
...
mod tests {
    ...
    async fn test_json_migration() {
        ...
        tool.extract_obs_credentials(&session.session_id, &test_config)
            .await
            .unwrap();

        // Verify migration
        let migrated_password = tool
            .config_manager
            .get_config(&session.session_id, "obs.OBS_REC.password")
            .await
            .unwrap();
    ...
    async fn test_migration_verification() {
        ...
        tool.migrate_hardcoded_credentials(&session.session_id)
            .await
            .unwrap();
