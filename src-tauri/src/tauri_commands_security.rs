...
use crate::security::{
    AccessLevel, ConfigCategory, ConfigMigrationTool, MigrationConfig, MigrationStats,
    SecureConfigManager, SecurityError,
};
...
pub async fn security_verify_migration(
    ...
    // Verify migration
    let is_verified = migration_tool
        .verify_migration(&session_id)
        .await
        .map_err(TauriSecurityError::from)?;
...
pub async fn security_create_session(
    ...
    log::info!(
        "Creating security session for user: {}",
...
pub async fn security_get_config(
    ...
    let value = config_manager
        .get_config(&config_request.session_id, &config_request.key)
        .await
        .map_err(TauriSecurityError::from)?;
...
pub async fn security_delete_config(
    ...
    let deleted = config_manager
        .delete_config(&session_id, &config_key)
        .await
        .map_err(TauriSecurityError::from)?;
...
pub async fn security_list_config_keys(
    ...
    let keys = config_manager
        .list_config_keys(&session_id, config_category)
        .await
        .map_err(TauriSecurityError::from)?;
...
pub async fn security_get_audit_history(
    ...
    let entries = if let Some(key) = config_key {
        audit
            .get_config_audit_history(&key, limit)
            .await
            .map_err(TauriSecurityError::from)?
    } else if let Some(user) = user_context {
        audit
            .get_user_audit_history(&user, limit)
            .await
            .map_err(TauriSecurityError::from)?
    } else {
        // Get recent security events
        audit
            .get_security_events(24) // Last 24 hours
            .await
            .map_err(TauriSecurityError::from)?
    };

    // Convert to response format
    let responses: Vec<AuditResponse> = entries
        .into_iter()
        .map(|entry| AuditResponse {
            id: entry.id,
...
pub async fn security_test_system(
    ...
    let retrieved_value = config_manager
        .get_config(&session.session_id, test_key)
        .await
        .map_err(TauriSecurityError::from)?;
    ...
    // Clean up test data
    config_manager
        .delete_config(&session.session_id, test_key)
        .await
        .map_err(TauriSecurityError::from)?;
