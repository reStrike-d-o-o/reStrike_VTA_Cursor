        ...
        if let Some(name) = &filename {
            ...
            if let Ok(conn2) = self.database_plugin().get_connection().await {
                ...
                let _ = if let Some(dbid) = match_id_db {
                    conn_ref2.execute(
                        "INSERT INTO recorded_videos (match_id, event_id, tournament_id, tournament_day_id, video_type, file_path, record_directory, filename_formatting, start_time, duration_seconds, created_at, created) VALUES (?, NULL, (SELECT uuid FROM tournaments WHERE id = ?), (SELECT uuid FROM tournament_days WHERE id = ?), 'replay', ?, ?, NULL, ?, ?, ?, strftime('%s','now'))",
                    ...
                    conn_ref2.execute(
                        "INSERT INTO recorded_videos (match_id, event_id, tournament_id, tournament_day_id, video_type, file_path, record_directory, filename_formatting, start_time, duration_seconds, created_at, created) VALUES ((SELECT id FROM pss_matches ORDER BY created_at DESC LIMIT 1), NULL, (SELECT uuid FROM tournaments WHERE id = ?), (SELECT uuid FROM tournament_days WHERE id = ?), 'replay', ?, ?, NULL, ?, ?, ?, strftime('%s','now'))",
