...
impl DatabaseMaintenance {
    ...
    pub async fn run_full_maintenance(
        &mut self,
        db_conn: &DatabaseConnection,
    ) -> DatabaseResult<MaintenanceResult> {
        let start_time = Instant::now();
...
impl MaintenanceNeeded {
    ...
    pub fn any_needed(&self) -> bool {
        self.vacuum_needed
            || self.integrity_check_needed
            || self.analyze_needed
            || self.optimize_needed
    }
}
