...
impl UiSettingsOperations {
    ...
    pub fn initialize_ui_settings(conn: &mut Connection) -> DatabaseResult<()> {
        ...
        let ui_settings = vec![
            ...
            (
                ...
                "integer",
                Some("300"),
                Some(r#"{"min": 200, "max": 500}"#),
            ),
            (
                "layout.status_bar_visible",
                "Status Bar Visible",
                "boolean",
                Some("true"),
                None,
            ...
            (
                "animations.duration_ms",
                "Animation Duration",
                "integer",
                Some("300"),
                Some(r#"{"min": 0, "max": 2000}"#),
            ),
            (
    ...
    pub fn set_ui_setting(
        ...
        if let Some(existing) = existing_value {
            ...
            // Create new value
            let setting_value =
                SettingsValue::new(setting_key.id.clone().unwrap(), value.to_string());
