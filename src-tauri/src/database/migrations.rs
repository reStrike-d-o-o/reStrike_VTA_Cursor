...
impl Migration for Migration37 {
    ...
    fn down(&self, conn: &Connection) -> SqliteResult<()> {
        // Best-effort rollback: rename pss_events back to pss_events_v2 if no conflicting table
        ...
        // Restore legacy backup if present and pss_events_v2 not in use
        let backup_exists: bool = conn
            .prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='pss_events_legacy_backup'")?
            .exists([])?;
        if backup_exists {
            let _ = conn.execute("DROP TABLE IF EXISTS pss_events", []);
            let _ = conn.execute(
