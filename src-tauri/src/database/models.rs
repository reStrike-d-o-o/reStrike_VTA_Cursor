...
pub struct EventTrigger {
    ...
    pub action_kind: Option<String>, // scene|overlay|record_start|record_stop|replay_save|hotkey|stinger
    pub obs_connection_name: Option<String>,
    pub condition_round: Option<i64>,
    pub condition_once_per: Option<String>, // 'round'|'match'
...
impl EventTrigger {
    pub fn new(
        ...
        Self {
            ...
            condition_round: None,
            condition_once_per: None,
            debounce_ms: Some(0),
            cooldown_ms: Some(0),
            is_enabled: true,
            priority,
    ...
    pub fn from_row(row: &Row) -> rusqlite::Result<Self> {
        Ok(Self {
            ...
            obs_connection_name: row.get("obs_connection_name")?,
            condition_round: row.get("condition_round")?,
            condition_once_per: row.get("condition_once_per")?,
            debounce_ms: row.get("debounce_ms")?,
            cooldown_ms: row.get("cooldown_ms")?,
