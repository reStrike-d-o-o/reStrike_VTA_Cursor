...
impl DrivePlugin {
    ...
    pub async fn get_quota(&self) -> AppResult<(u64, u64, u64)> {
        ...
        let usage = quota["usage"]
            ...
            .and_then(|s| s.parse::<u64>().ok())
            .unwrap_or(0);
        let usage_in_drive = quota["usageInDrive"]
            .as_str()
            .and_then(|s| s.parse::<u64>().ok())
            .unwrap_or(0);
    ...
    pub async fn upload_file_streaming_to_folder(
        ...
        let session_uri = initiate
            .headers()
            .get("location")
            .or_else(|| initiate.headers().get("Location"))
            .and_then(|v| v.to_str().ok())
            .ok_or_else(|| AppError::ConfigError("No Location header".to_string()))?
            .to_string();
        ...
        while sent < file_size {
            ...
            let n =
                std::io::Read::read(&mut file, &mut buf[..to_read]).map_err(AppError::IoError)?;
            let start = sent;
    ...
    pub async fn exchange_code(&self, code: String) -> AppResult<()> {
        ...
        let client = client_guard
            .as_ref()
            .ok_or_else(|| AppError::ConfigError("Client not initialized".to_string()))?;
        ...
        if let Some(refresh) = token_res.refresh_token() {
            ...
            let expires_at = std::time::SystemTime::now()
                ...
                .unwrap()
                .as_secs()
                + token_res.expires_in().unwrap().as_secs();
    ...
    pub async fn list_files(&self) -> AppResult<Vec<GoogleDriveFile>> {
        ...
        let response = self
            ...
            .query(&[
                (
                    "q",
                    "mimeType='application/zip' and name contains 'reStrikeVTA_backup'",
                ),
                (
                    "fields",
                    "files(id,name,mimeType,size,createdTime,modifiedTime)",
    ...
    pub async fn upload_backup_archive(&self) -> AppResult<String> {
        ...
        match self
            .create_backup_archive(&current_dir, &archive_path)
            .await
        {
        ...
        let _file_id = match self
            .upload_file_streaming(&archive_path, &archive_name)
            .await
        {
            Ok(file_id) => {
            ...
            Err(e) => {
                ...
                self.log_error_comprehensively("upload_backup_archive", &error_msg, Some(&details));
    ...
    pub async fn download_backup_archive(&self, file_id: &str) -> AppResult<String> {
        ...
        self.extract_backup_archive(&download_path, &backup_dir)
            .await?;
    ...
    async fn extract_backup_archive(
        &self,
        archive_path: &std::path::Path,
        extract_dir: &std::path::Path,
    ) -> AppResult<()> {
        let file = std::fs::File::open(archive_path).map_err(|e| AppError::IoError(e))?;

        let mut archive =
            ZipArchive::new(file).map_err(|e| AppError::ConfigError(e.to_string()))?;
        ...
        for i in 0..archive.len() {
            ...
            if file.name().ends_with('/') {
                std::fs::create_dir_all(&outpath).map_err(|e| AppError::IoError(e))?;
            } else {
                if let Some(p) = outpath.parent() {
                    if !p.exists() {
                        std::fs::create_dir_all(p).map_err(|e| AppError::IoError(e))?;
                    }
                }
                let mut outfile =
                    std::fs::File::create(&outpath).map_err(|e| AppError::IoError(e))?;
                std::io::copy(&mut file, &mut outfile).map_err(|e| AppError::IoError(e))?;
            }
        }
    ...
    pub async fn upload_file_streaming(
        ...
        log::info!(
            ...
            &token[token.len().saturating_sub(10)..]
        );
        ...
        let initiate_response = match request_builder.send().await {
            ...
            Err(e) => {
                let error_msg =
                    format!("Failed to send resumable upload initiation request: {}", e);
                log::error!("{}", error_msg);
                log_error_to_file(&error_msg);
                self.log_error_comprehensively(
                    "upload_file_streaming",
                    ...
                    Some(&format!(
                        "URL: {}, File: {}, Size: {} bytes",
                        initiate_url, file_name, file_size
                    )),
                );
        ...
        if !status.is_success() {
            ...
            let details = format!(
                "File: {}\nSize: {} bytes\nStatus: {}\nURL: {}\nResponse: {}",
                file_name, file_size, status, initiate_url, error_text
            );
            ...
            match status.as_u16() {
                401 => {
                    return Err(AppError::ConfigError(
                        "Authentication failed - access token may be expired".to_string(),
                    ))
                }
                403 => {
                    return Err(AppError::ConfigError(
                        "Permission denied - check Google Drive API access".to_string(),
                    ))
                }
                429 => {
            ...
            .and_then(|v| v.to_str().ok())
        {
            ...
            None => {
                ...
                let all_headers: Vec<String> = initiate_response
                    .headers()
                    .iter()
        ...
        if !session_uri.starts_with("https://") || !session_uri.contains("upload_id=") {
            ...
            self.log_error_comprehensively(
                "upload_file_streaming",
                &error_msg,
                Some(&format!("Expected format: https://...upload_id=...")),
            );
            return Err(AppError::ConfigError(error_msg));
        }
        ...
        loop {
            ...
            let bytes_read = {
                ...
                match file.read(&mut chunk) {
                    ...
                    Err(e) => {
                        let error_msg =
                            format!("Failed to read file chunk {}: {}", chunk_count + 1, e);
                        log::error!("{}", error_msg);
                        log_error_to_file(&error_msg);
                        self.log_error_comprehensively(
                            "upload_file_streaming",
            ...
            if bytes_read == 0 {
                log::info!(
                    "File reading complete - uploaded {} chunks totaling {} bytes",
                    chunk_count,
                    uploaded
                );
                break; // End of file
                    ...
                    Ok(response) => {
                        ...
                        match status.as_u16() {
                            200 | 201 => {
                                // Upload complete!
                                log::info!(
                                    "Upload completed successfully after {} chunks!",
                                    chunk_count
                                );
                                let file_data: serde_json::Value =
                                    response.json().await.map_err(|e| {
                                        let error_msg =
                                            format!("Failed to parse final upload response: {}", e);
                                        self.log_error_comprehensively(
                                            "upload_file_streaming",
                                            &error_msg,
                                ...
                                let file_id = file_data["id"]
                                    ...
                                    .ok_or_else(|| {
                                        ...
                                        AppError::ConfigError(
                                            "No file ID in upload response".to_string(),
                                        )
                                    })?;
                            ...
                            308 => {
                                ...
                                log::info!(
                                    "Chunk {} uploaded successfully, continuing...",
                                    chunk_count
                                );
                                uploaded += bytes_read as u64;
                    ...
                    Err(e) => {
                        ...
                        if retry_count > max_retries {
                            let error_msg = format!(
                                ...
                                chunk_count, max_retries, e
                            );
                            self.log_error_comprehensively(
                                "upload_file_streaming",
                                &error_msg,
                                Some(&format!(
                                    "File: {}, Session URI: {}, Bytes: {}-{}",
                                    file_name, session_uri, uploaded, chunk_end
                                )),
                            );
        ...
        self.log_error_comprehensively(
            ...
            error_msg,
            Some(&format!(
                "File: {}, Uploaded: {} bytes, Chunks: {}",
                file_name, uploaded, chunk_count
            )),
        );
    ...
    async fn download_file(&self, file_id: &str) -> AppResult<std::path::PathBuf> {
        ...
        std::fs::write(&temp_path, content).map_err(|e| AppError::IoError(e))?;
